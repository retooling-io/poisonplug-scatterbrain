...
def recover_cfg_step(
    ...
    def _step(
        ...
        try:                                 # exceptions can throw on decoding
            ...
            if s.log:
                print(f'\t[RecoverCfg] {s.prev_instr}')
...
def recover_cfg_emu(
    ...
    """
        emu = EmulateIntel64()
        emu.map_shellcode(bytes(self.d.imgbuffer))
        emu.map_teb()
...
def normalize_raw_recovery(
    ...
    for i, r in enumerate(s.recovered):
        ...
        if not fall_through_ea in s.ea_to_recovered:
            #_LOG_FAILED_FALLTHROUGH_(fall_through_ea)
        ...
        if (
            ...
            assert fall_through_ea in s.ea_to_linear # @NOTE: some JCC edge case I hit w/o recalling?
            #----------------------------------------------------------------------
