...
Everything else uses these primitives to carry out the deobfuscation.
`deobfuscator.py` should be the routine that puts all of the transformations
together to produce the final binary:
    - `deobfuscator64.py`
    - `recover_<TRANSFORMATION_PASS>`.py
        - should be the naming convention for all transformations
            \\_ recover_dispatchers.py
            \\_ recover_imports.py
            \\_ recover_cfg.py
                \\_ recover_func.py
                \\_ recover_impstub.py
            \\_ recover_img64.py
                \\_ build_imgtemplate.py
                \\_ build_relocations.py
...
# since it's ultimately a task of instruction recovery, give them types
# associated with the protection
class InstructionLabel:
    Invalid:         int = -1
    Normal:          int = 0   # Original instruction
    ProtectedImport: int = 1   # obfuscator backbone: ids protected imports calls
    Dispatcher:      int = 2   # obfuscator backbone: ids instr dispatcher calls/jmps
    TestOP:          int = 3   # obfuscator backbone: ids test opaque predicates
    JccOP:           int = 4   # obfuscator backbone: ids jcc nonsense
    SyntheticJmp:    int = 5   # user generated synthetic jmp

class Instruction(x86Instr):
    tag: int = InstructionLabel.Invalid
...
class RecoveredInstr:
    ...
    # @TODO(new_wip):
    tag:             int        = InstructionLabel.Invalid
    is_obf_import:   bool       = False # @TODO(deprecate for tag above)
    ...
    def _initialize_image_input(self):
        ...
        try:
            ...
            if self.protection_type == ProtectionType.HEADERLESS:
                ...
                # @NOTE: ^^ doesn't hold for Fujitsu case, maybe I chopped some bytes there?? But
...
"""
Rules will be specific
"""

# @NOTE: __ORDER_MATTERS__
...
def RULE_HANDLE_TEST_OPAQUE_PREDICATE(
    ...
    def _verify_test_op():
        ...
        if not (
            ...
            instr.get_op2_imm() == 0   # the immediate is always 0
        ):
            return False,None
...
def RULE_HANDLE_RET_INT3(
    ...
    if instr.is_ret() or instr.is_int3():
        # Handle ret and int3 instructions
        s.recovered.append(
            RecoveredInstr(
