...
try:
    ...
    print("Make sure you have the 'pefile' library installed.")
    print("You can install it using: `pip install pefile`")
    exit(1)
...
def build_memory_image(
    ...
    """
    #---------------------------------------------------------------------------
    # @TODO: never use this `pe` api again, it does the base minimum and also
    #        doesn't clear header data
...
def build_memory_image_with_imports(
    ...
    """
    Create a new pefile from an existing one with a new import section appended
    at the end of it. The data within the existing pefile i.e, text section etc.
    is left intact. If you want a cleared text section (i.e., when rebuilding
    fully protected samples) set the clear_text flag.

    @NOTE: This was quick win code, perhaps revist at some point but fine for
           the given use case
    ...
    # @TODO: log, but given we control this, this will always be true anyway
...
# the ones set with 0xFFFFFFF markers in template are expected to be updated
@dataclass
...
def build_from_headerless_image_with_imports(
    ...
    Logic is centralized around the fact that all known headerless inputs
    have consisted solely of .text and .data section. Nothing outside of
    these two has been seen. The logic here goes with that assumption and
    further adds a new, recovered import directory alongside it.
    """
